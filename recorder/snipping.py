...
Alle angeschlossenen Monitore werden unterstuetzt (monitors[0] = virtuelle Gesamtflaeche).
HiDPI-Skalierung wird behandelt.
"""
...
class SnippingTool:
    def capture_area(self, parent: tk.Tk) -> Optional[str]:
        """
        Vollbild-Overlay ueber alle Monitore.
        Rueckgabe: Base64-JPEG des ausgewaehlten Bereichs oder None bei Abbruch.
        """
        # 1. Alle Monitore als eine virtuelle Flaeche aufnehmen
        with mss.mss() as sct:
            mon     = sct.monitors[0]   # 0 = kombinierte Flaeche aller Monitore
            raw     = sct.grab(mon)
            mon_left = mon["left"]
            mon_top  = mon["top"]
            mon_w    = mon["width"]
        ...
        # 2. HiDPI-Skalierung: physische Pixel vs. logische Pixel
        scale_x = img_full.width  / mon_w
        scale_y = img_full.height / mon_h

        if abs(scale_x - 1.0) > 0.05 or abs(scale_y - 1.0) > 0.05:
            img_display = img_full.resize((mon_w, mon_h), Image.LANCZOS)
        else:
        ...
        # 3. Overlay-Fenster ueber alle Monitore
        ...
        canvas = tk.Canvas(
            overlay, width=mon_w, height=mon_h,
        ...
        def on_drag(e):
            ...
            if bx1 > bx0 and by1 > by0:
                ...
                canvas._crop_ph = ph
                state["bright_id"] = canvas.create_image(bx0, by0, anchor="nw", image=ph)
